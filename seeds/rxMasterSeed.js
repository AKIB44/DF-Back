...
// Maps service name (from main seed) → svc_id code used by rx master data
const SERVICE_CODES = [
...
const PROCEDURES = [
  ...
  { procedure_code: 'IMP-PLACE',  procedure_name: 'Implant fixture placement',                    svc_id: 'SVC-06', procedure_step: 1,    duration_days: 7,  followup_days: 90,  default_notes: 'Swelling and bruising normal 3-5 days. Soft foods only 2 weeks. No smoking critical for osseointegration. NO hard biting on implant site for 90 days.' },
  ...
  { procedure_code: 'PULP-PULP',  procedure_name: 'Pulpectomy and medicated dressing',            svc_id: 'SVC-07', procedure_step: 1,    duration_days: 2,  followup_days: 7,   default_notes: 'Child may have mild soreness 1-2 days. Give paracetamol syrup if uncomfortable. Bring child back if severe swelling or fever.' },
...
// svc_id → [generic_name, sort_order]
...
async function run() {
  // 1. Stamp service codes onto the services table (matched by name)
  for (const [name, code] of SERVICE_CODES) {
    await db.query(
      `UPDATE services SET code = $1 WHERE name = $2 AND code IS NULL`,
      [code, name]
    );
  }
  console.log('Service codes updated.');

  // 2. Seed medicines (skip if already present by generic_name)
  const medIds = {};
  for (const m of MEDICINES) {
    const existing = await db.query(
      `SELECT id FROM rx_medicines WHERE generic_name = $1`,
      [m.generic_name]
    );
    ...
    const result = await db.query(
      `INSERT INTO rx_medicines
       ...
       RETURNING id`,
      [m.generic_name, m.brand_name, m.category, m.dosage_form, m.strength,
       m.default_dose, m.default_days ?? null, m.notes]
    );
    medIds[m.generic_name] = result.rows[0].id;
  }
  console.log(`Medicines: ${Object.keys(medIds).length} ready.`);

  // 3. Seed procedures (skip if already present by procedure_code)
  for (const p of PROCEDURES) {
    await db.query(
      ...
      [p.procedure_code, p.procedure_name, p.svc_id, p.procedure_step ?? null,
       p.default_notes, p.duration_days, p.followup_days ?? null]
    );
  }
  ...
  for (const [svcId, pairs] of Object.entries(SERVICE_DEFAULTS)) {
    for (const [genericName, sortOrder] of pairs) {
      ...
      if (!medId) continue;
      await db.query(
        `INSERT INTO rx_service_defaults (svc_id, medicine_id, sort_order)
         VALUES ($1,$2,$3)
         ON CONFLICT (svc_id, medicine_id) DO NOTHING`,
        [svcId, medId, sortOrder]
      );
    }
  }
  console.log('Service defaults ready.');
  console.log('Rx master seed complete.');
}
